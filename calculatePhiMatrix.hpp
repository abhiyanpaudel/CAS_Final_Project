 ...
 * The Phi matrix represents the evaluation of basis functions at particle
 * positions.
 */
 ...
 */
Omega_h::LOs
find_element_from_source_points(Omega_h::Mesh &mesh,
  ...
  Kokkos::View<pcms::Real *[2]> source_points("points to find the elements",
                                              nsources);

  Omega_h::parallel_for(
      nsources, OMEGA_H_LAMBDA(const int i) {
  ...
  Omega_h::Write<Omega_h::LO> element_ids(
      nsources, -1, "stores the element ids for corresponding source points");
  pcms::GridPointSearch search_element(mesh, 10, 10);
  auto results = search_element(source_points);

  Omega_h::parallel_for(
      nsources,
      OMEGA_H_LAMBDA(const int id) { element_ids[id] = results(id).tri_id; });
 ...
 */
struct Results {
  Kokkos::View<PetscInt *[3]>
      row_ids; /**< Row indices for each source point and vertex */
  Kokkos::View<PetscScalar *[3]>
      vals; /**< Shape function values for each source point and vertex */
};
 ...
 * vertices (row indices), and computes the barycentric coordinates (shape
 * function values) of the source point within that element.
 *
 * @param mesh The target Omega_h mesh
 * @param source_coordinates Coordinates of the source points
 * @return Results Structure containing row indices and shape function values
 */
Results get_row_ids_and_values(Omega_h::Mesh &mesh,
                               const Omega_h::Reals &source_coordinates) {
  ...
  std::cout << "DEBUG: Computing shape functions and filling arrays"
            << std::endl;
  Omega_h::parallel_for(
      nsources, OMEGA_H_LAMBDA(const int id) {
        auto current_src_el_id = element_ids[id];

        const auto current_el_verts =
            Omega_h::gather_verts<3>(faces2nodes, current_src_el_id);
        ...
        Omega_h::Vector<2> global_coordinate{source_coordinates[id * dim + 0],
                                             source_coordinates[id * dim + 1]};

        // Calculate barycentric coordinates
        auto barycentric_coordinate = barycentric_from_global<2, 2>(
            global_coordinate, current_el_vert_coords);

        if (id <
            10) { // Only print for the first 10 points to avoid too much output
          printf("Point %d barycentric: [%f, %f, %f]\n", id,
                 barycentric_coordinate[0], barycentric_coordinate[1],
...
/**
 * @brief Calculates the Phi matrix for mapping from source points to a target
 * mesh
 *
 * Creates a sparse matrix that is used to map field values from source points
 * to the target mesh. The matrix is constructed in CSR format using row indices
 * and values from shape functions. This implements a finite element projection
 * of point data onto the mesh.
 *
 * Algorithm:
...
calculatePhiMatrix(Omega_h::Mesh &mesh,
                   const Omega_h::Reals &source_coordinates, Mat *Phi_out) {
  ...
  for (int i = 0; i <= std::min(nrows, 10); ++i)
    std::cout << h_ia[i] << " ";
  std::cout << "\n";
  ...
  Omega_h::parallel_for(nrows, OMEGA_H_LAMBDA(int i) { next(i) = ia(i); });
  Kokkos::fence();
  ...
  for (int i = 0; i < nnz; ++i) {
    if (h_ja[i] < 0) {
      std::cerr << "ERROR: negative entry at i=" << i << " ja=" << h_ja[i]
                << " a=" << h_a[i] << "\n";
      break;
    }
  }
  ...
  Mat Phi;
  PetscCall(MatCreateSeqAIJKokkosWithKokkosViews(PETSC_COMM_SELF, nrows, ncols,
                                                 ia, ja, a, &Phi));
  *Phi_out = Phi;
