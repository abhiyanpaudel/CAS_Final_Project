...
static PetscErrorCode CreateMatrix(Omega_h::Mesh &mesh, Mat *A) {
  ...
  PetscCall(MatSetSizes(*A, mesh.nverts(), mesh.nverts(), PETSC_DECIDE,
                        PETSC_DECIDE));
...
inline PetscErrorCode calculateMassMatrix(Omega_h::Mesh &mesh, Mat *mass_out) {
  ...
  PetscBool is_kokkos;
  PetscCall(PetscObjectBaseTypeCompare((PetscObject)mass, MATSEQAIJKOKKOS,
                                       &is_kokkos));
  std::cerr << "Matrix type is kokkos: " << is_kokkos << "\n";
  PetscCall(MatZeroEntries(mass));
  PetscCall(MatSetValuesCOO(
      mass, elmMassMatrix.data(),
      INSERT_VALUES)); // FIXME fails here on gpu, calls into host
                       // implementation... AFAIK, petsc checks the type of the
                       // input array of values to decide which backend to
                       // use...
  //
  if (mesh.nelems() < 10) {
