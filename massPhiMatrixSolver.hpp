 ...
 * @date May 1, 2025
 *
 * This file contains functions to solve the linear system Mass * x = Phi *
 * source_values for mapping field values from source points to a target mesh
 * using a finite element approach with PETSc and Kokkos for GPU acceleration.
 */
...
static Vec createKokkosVec(PetscInt len, const Omega_h::Reals &data) {
  std::cout << "DEBUG: createKokkosVec - Creating vector of length " << len
            << std::endl;
  Vec v;
  PetscErrorCode ierr;
  ...
  std::cout << "DEBUG: createKokkosVec - Vector created, now filling with data"
            << std::endl;
  PetscScalar *array;
...
static Vec multiplyMatVec(Mat M, Vec u) {
  ...
  CHKERRABORT(PETSC_COMM_WORLD, ierr);
  std::cout << "DEBUG: multiplyMatVec - Matrix size: " << m << " x " << n
            << std::endl;
  ...
  std::cout << "DEBUG: multiplyMatVec - Performing matrix-vector multiplication"
            << std::endl;
  ierr = MatMult(M, u, b);
  CHKERRABORT(PETSC_COMM_WORLD, ierr);
...
static Vec solveLinearSystem(Mat A, Vec b) {
  ...
  CHKERRABORT(PETSC_COMM_WORLD, ierr);
  std::cout << "DEBUG: solveLinearSystem - Matrix size: " << m << " x " << n
            << std::endl;
...
static Mat buildMassMatrixKokkos(Omega_h::Mesh &mesh) {
  std::cout << "DEBUG: buildMassMatrixKokkos - Building mass matrix"
            << std::endl;
  Mat mass;
...
/**
 * @brief Constructs the Phi matrix for mapping from source points to target
 * mesh
 *
 ...
 * @return Omega_h::Reals Solution vector representing mapped values at mesh
 * vertices
 * @throws std::runtime_error If source_values size doesn't match
 * source_coordinates
 */
Omega_h::Reals solveMassPhiSystem(Omega_h::Mesh &mesh,
  ...
  std::cout << "DEBUG: solveMassPhiSystem - Source points: " << ncols
            << std::endl;
  ...
  std::cout << "DEBUG: solveMassPhiSystem - Creating vectors and solving system"
            << std::endl;
  ...
  Omega_h::Write<Omega_h::Real> solution_vector(
      ncols, 0, "stores the solution coefficients");
  {
    PetscErrorCode ierr;
    PetscScalar *array;
